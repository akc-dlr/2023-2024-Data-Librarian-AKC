...
def init():
	...
	st.session_state["engine"] = pt.BatchRetrieve(index, wmodel="BM25")
...
def search(query):
	...
	for _, row in res.iterrows():
		...
		for field in fields_to_show:
			if 	field == "text":
				st.markdown(f"<h1 style='font-size: 16px;'>{entry[field]}</h1>", unsafe_allow_html=True)
			elif field == "source":
				source_url = entry[field]
				st.markdown(f"<a href='{source_url}' target='_blank'>{source_url}</a>", unsafe_allow_html=True)
			else:
				st.markdown(f"<div style='font-size: 16px;'>{field.capitalize()}: \t {entry[field]}</div>", unsafe_allow_html=True)
